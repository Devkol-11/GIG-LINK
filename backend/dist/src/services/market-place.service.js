import { ApplicationStatus, ContractStatus, ROLE_USER, } from "../../prisma/generated/prisma/enums.js";
import { marketPlaceRepository } from "../repositories/market-place.repository.js";
import { AppError } from "../utils/error.js";
export const marketPlaceService = {
    async createGig(userId, role, data) {
    ...
    async updateGig(userId, role, id, data) {
        if (role !== ROLE_USER.CREATOR) {
    ...
    async deleteGig(userId, role, id) {
        if (role !== ROLE_USER.CREATOR) {
    ...
    async createApplication(userId, role, data) {
    ...
    listApplications(userId, role) {
        return marketPlaceRepository.listApplicationsForUser(userId, role);
    },
    async updateApplicationStatus(userId, role, applicationId, status) {
        if (role !== ROLE_USER.CREATOR) {
        ...
        return marketPlaceRepository.updateApplicationStatus(applicationId, status === "accepted" ? ApplicationStatus.ACCEPTED : ApplicationStatus.REJECTED);
    },
    async createContract(userId, role, data) {
        if (role !== ROLE_USER.CREATOR) {
    ...
    async updateContractStatus(userId, role, id, status) {
        const contract = await marketPlaceRepository.getContractById(id);
        if (!contract) {
            throw new AppError("Contract not found", 404);
        }
