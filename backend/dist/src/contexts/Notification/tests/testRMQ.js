        ...
        try {
                ...
                for (const e of events) {
                        winston_1.logger.info(
                                `Publishing: ${JSON.stringify(e.payload)}`
                        );
                        await RabbitMQ_1.rabbitMQService.publish(
                                e.exchange,
                                e.routing_key,
                                e.payload
                        );
...
const consumeTest = async () => {
        try {
                await RabbitMQ_1.rabbitMQService.connect();
                await RabbitMQ_1.rabbitMQService.consume(
                        'app_exchange',
                        'User',
                        'array_test_queue',
                        async (msg) => {
                                ...
                                winston_1.logger.info(
                                        `Simulating email sending to: ${msg.email}...`
                                );
                        }
                );
        } catch (error) {
