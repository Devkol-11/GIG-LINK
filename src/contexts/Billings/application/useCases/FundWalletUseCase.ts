...
export class FundWalletUseCase {
  ...
  async Execute(
    ...
    amount: number,
    email: string,
    opts?: { currency?: string; channel?: PaymentChannelType }
  ) {
    // --- Basic validation
    ...
    if (userId !== wallet.userId)
      throw BusinessError.unauthorized("unauthorized: invalid wallet");
    ...
    if (amount > MAX_FUND_NAIRA)
      throw BusinessError.forbidden("exceeds maximum funding amount");
    ...
    try {
      providerResponse = await this.paymentProvider.initializePayment({
    ...
    if (providerResponse?.reference) {
      ...
      await this.transactionRepository.save(tx).catch(() => null);
    }
