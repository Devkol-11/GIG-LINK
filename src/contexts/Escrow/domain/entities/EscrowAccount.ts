...
export class EscrowAccount {
  ...
  public static create(
    props: Omit<EscrowAccountProps, "id" | "createdAt" | "updatedAt">
  ): EscrowAccount {
  ...
  fund(amount: number) {
    if (amount <= 0)
      throw BusinessError.forbidden("Amount must be greater then zero");
  ...
  release(amount: number) {
    ...
    if (amount > this.props._balanceCents)
      BusinessError.forbidden("insufficient funds in escrow account");
    this.props._balanceCents -= amount;
  }
  lock() {
