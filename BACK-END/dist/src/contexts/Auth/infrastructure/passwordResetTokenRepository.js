...
class PasswordResetTokenRepository {
    ...
    toDomainEntity(record) {
        return new PasswordResetToken_1.PasswordResetToken(record.id, record.userId, record.token, record.used, record.createdAt, record.expiresAt);
    }
    ...
    async create(userId, ttlMinutes = 15) {
        ...
        const entity = new PasswordResetToken_1.PasswordResetToken((0, cuid2_1.createId)(), userId, (0, generateOTP_1.generateOtp)(), false, now, expiresAt);
        const record = await prismaClient_1.prisma.passwordResetToken.create({
            data: this.toPersistence(entity),
        });
        return this.toDomainEntity(record);
    }
    async findByToken(token) {
        const record = await prismaClient_1.prisma.passwordResetToken.findUnique({
            where: { token },
        });
    ...
    async deleteAllForUser(userId) {
        await prismaClient_1.prisma.passwordResetToken.deleteMany({ where: { userId } });
    }
}
