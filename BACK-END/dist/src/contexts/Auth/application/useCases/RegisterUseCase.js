...
class RegisterUseCase {
    ...
    async Execute(DTO) {
        ...
        if (await this.authRepository.findByEmail(email)) {
            throw new DomainException_1.DomainException(`User : ${email} Already Exists`, 404);
        }
        ...
        const accessToken = this.authservice.generateAccessToken(newUser.id, newUser.id);
        const userRegisteredEvent = new UserRegisteredEvent_1.UserRegisteredEvent(newUser.id, newUser.email, newUser.firstName, newUser.lastName);
        await this.eventPublisher.publish(userRegisteredEvent.routing_key, userRegisteredEvent);
        return {
...
exports.registerUseCase = new RegisterUseCase(AuthService_1.authservice, AuthRepository_1.authRepository, RabbitMQService_1.rabbitMQEventPublisher);
